...
class YoukuSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        for div in div_list:
            ...
            title = div.xpath('.//div[@class="v-link"]/a/@title').extract_first()

            video_url = div.xpath('.//div[@class="v-link"]/a/@href').extract_first()
            url = "http:" + video_url

            video_time = div.xpath('.//div[@class="v-thumb"]//span/text()').extract_first()

            play_count = div.xpath('.//span[@class="pub"]/text()').extract_first()
